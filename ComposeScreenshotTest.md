...
> **Single source of truth — update this value when upgrading the plugin, then all code snippets below stay correct automatically.**

```
SCREENSHOT_PLUGIN_VERSION = 0.0.1-alpha14
```

Wherever you see `$SCREENSHOT_PLUGIN_VERSION` in this document, substitute the value above.
...
- Current stable alpha: `$SCREENSHOT_PLUGIN_VERSION` or find the latest version
...
| Screenshot plugin | ≥ `$SCREENSHOT_PLUGIN_VERSION` | ≥ `$SCREENSHOT_PLUGIN_VERSION` |
...
screenshot = "$SCREENSHOT_PLUGIN_VERSION"
...
```kotlin
plugins {
    id("com.android.compose.screenshot") version "$SCREENSHOT_PLUGIN_VERSION" apply false
}
```
...
```kotlin
dependencies {
    screenshotTestImplementation("com.android.tools.screenshot:screenshot-validation-api:$SCREENSHOT_PLUGIN_VERSION")
